...
* \`hover\`: Move the mouse cursor to the specified coordinates or element without clicking. Useful for revealing tooltips, dropdown menus, or triggering hover states.`,
  ...
  {
    name: 'search_history',
    description: `Search the user's browsing history by keyword and time range. Returns matching pages with URLs, titles, last visit time, and visit count. Useful for finding pages the user visited recently (e.g., "find the article about React hooks I read last week"). Leave query empty to get all recent history.`,
    input_schema: {
      type: 'object',
      properties: {
        query: {
          type: 'string',
          description: 'Text to search for in page titles and URLs. Leave empty to get recent history.',
        },
        max_results: {
          type: 'number',
        ...
        days_back: {
          type: 'number',
          description: 'How many days back to search (default: 30). For example, 7 means only the last week.',
        },
      },
      required: [],
    },
...

  {
    ...
    description: `Bookmark a URL. Defaults to the current page if no URL is provided. Optionally specify a title and a folder name. If the folder doesn't exist, it will be created under Other Bookmarks.`,
    input_schema: {
      type: 'object',
      properties: {
        url: {
          type: 'string',
          description: 'URL to bookmark. Optional — if omitted, bookmarks the current active tab.',
        },
        title: {
          type: 'string',
          description: 'Bookmark title. Optional — if omitted, uses the page title.',
        },
        folder: {
          type: 'string',
          description: 'Folder name to save the bookmark in. Optional — if omitted, saves to Other Bookmarks. If the folder does not exist, it will be created.',
        },
      },
      required: [],
    },
...

  {
    ...
    description: `Search the user's saved bookmarks by keyword. Returns matching bookmarks with URLs, titles, date added, and folder location.`,
    input_schema: {
      type: 'object',
      properties: {
        query: {
...

  {
    ...
    description: `Remove a bookmark by its URL. If multiple bookmarks have the same URL, all matching bookmarks are removed.`,
    input_schema: {
      type: 'object',
      properties: {
        url: {
          type: 'string',
          description: 'URL of the bookmark to remove.',
        },
      },
      required: ['url'],
    },
